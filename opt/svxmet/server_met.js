...
// const DTMF_PTY = "/tmp/test_ausgabe";
...
app.post("/api/dtmf", async (req, res) => {
 ...
 if (!digits || !/^[0-9*#]+$/i.test(digits)) {
  ...
  try {
    ...
    res.json({ ok: true, message: "Digits successfully sent to XXX device." });
  } catch (e) {
    console.error("[dtmf] Failed to write to XXX device:", e.message);
